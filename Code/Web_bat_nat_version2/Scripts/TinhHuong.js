function uploadSucFile(filename, dataname) {
    $('*[name="' + dataname + '"]').css('background-image', 'url("../images/upload/' + filename + '")').attr('valfile', filename);
}
var indexTotal = 0;
    ...
    if (strucdata.length > 0 && typeof strucdata[0].arrayQuestion != 'undefined') {
        $.each(strucdata, function (index, item) {
            ...
            $.each(item.arrayQuestion, function (indexQu, itemQe) {
                ...
                _dataPanelImg.attr('valfile', itemQe.itemImage.urlimage).css({ 'background-image': 'url("../images/upload/' + itemQe.itemImage.urlimage + '")', 'height': itemQe.itemImage.height + 'px' });
...
function createJsonSave() {
    if (!$('.contentInfoDeThi').validate()) {
        jConfirmLinkTam('Thông báo', '<div style="text-align:center;margin:15px 0px;">Vui lòng nhập đầy đủ thông tin bài trắc nghiệm</div>', null, null, 350,null,true);
        return;
    }
    var dataSetting = $('.contentInfoDeThi').getdatafrom();
    var _arraySave = new Array();
    $('.itemNgay').each(function (indexEx) {
        var _arrayQuestion = new Array();
        $(this).find('.itemTinhHuong').each(function (indexQuestion) {
            var _type = parseInt($(this).find('.itemTinhHuong-List-Type').attr('valitem'));

            //            CKEDITOR.replace('textEditor_panelKeoTha_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            //            CKEDITOR.replace('textEditor_panelDienCau_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            //            CKEDITOR.replace('textEditor_panelMatching_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            //            CKEDITOR.replace('textEditor_panelPhanLoai_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            //            CKEDITOR.replace('textEditor_ChonLuaDapAn_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            //            CKEDITOR.replace('textEditor_Image_' + _index, { language: 'vi', filebrowserUploadUrl: 'http://suphutienganh.com/ajax/uploadImage.ashx' });
            ...
            _dataImg.find('.admin-Image-Cau-Img-Data-Pixel-Item').each(function (indexPixel) {
                ...
                arrayPixel[indexPixel] = itemImagePixel;
            });
            var itemImage = {
                title: CKEDITOR.instances[$(this).find('.textarea_titleAll').attr('id')].getData(),
                urlimage: _dataImg.attr('valfile'),
            ...
            _arrayQuestion[indexQuestion] = itemQuestion;
        });
        var itemEx = {
            arrayQuestion: _arrayQuestion,
            title: CKEDITOR.instances[$(this).find('.parentTitle-Data').attr('id')].getData()
        };
        _arraySave[indexEx] = itemEx;
    });
    var dataPost = {
        setting: dataSetting,
        questions: _arraySave,
        id: _ID
    };
    jConfirmLinkTam('Thông báo', '<div style="text-align:center;margin:15px 0px;">Bạn chắc chắn muốn lưu bài?</div>', 'Chắc chắn', 'Hủy', 350, function (status) {
        if (status) {
            $('.popuplammo').css('display', 'block');
            setTimeout(function () {
                $.ajax({
                    type: "POST",
                    url: "../services/Quiz.asmx/saveQuiz",
                    data: "{'data':" + JSON.stringify(JSON.stringify(dataPost)) + "}",
                    ...
                    success: function (message) {
                        ...
                        if (!objJson[0]) {
                            jConfirmLinkTam('Thông báo', '<div style="text-align:center;margin:15px 0px;">' + objJson[1] + '</div>', null, null, 350, null, true);
                        } else {
                            window.location.href = 'Quiz.aspx?ID=' + objJson[1];
                        }
                    ...
                    error: function (errormessage) {
                        ...
                        $('.popuplammo').css('display', 'none');
                        jConfirmLinkTam('Thông báo', '<div style="text-align:center;margin:15px 0px;">Error ' + errormessage.status + ': ' + errormessage.statusText + '</div>', null, null, 350, null, true);
                    }
                });
            }, 1000);
        }
...
function createHtmlParent() {
    ...
    html += '           <div class="parentTitle-Title"><span>Tiêu đề Exercise</span></div>';
    ...
    html += '        </div>';
    html += '        <div style="bottom: 21px;position: absolute;right: 18px;"><span class="btnAll" style="background-image:url(\'../images/delete-icon.png\');margin-left:20px;">Xóa Exercise</span></div>';
...
function createHtmlTinhHuong() {
    ...
    html += '        <div class="itemTinhHuong-List-Type" valitem="4">';
    html += '           <span style="display:none;">Kéo thả câu khuyết</span>';
    html += '           <span style="display:none;">Gõ text hoàn thành</span>';
    html += '           <span style="display:none;">Matching hai nửa vế câu</span>';
    html += '           <span style="display:none;">Phân loại các từ</span>';
    html += '           <span style="display:block;">Lựa chọn đáp án</span>';
    html += '           <span style="display:none;">Kéo thả mô tả hình ảnh</span>';
    html += '        </div>';
    ...
    html += '                   <tr>';
    html += '                       <th style="text-align:left;width:75px;"></th>';
    html += '                       <td class="txtHuongDan">Lưu ý: Danh sách từ được chọn lựa sẽ nằm trong <b>{}</b>. Nhấn <b>Tab</b> để tạo <b>{}</b><br/>Trong {} sẽ có <b>n</b> đáp án, mỗi đáp án sẽ nằm trong <b>[]</b>.<br/>Đáp án đúng: <b>[đáp án-1]</b></br>Đáp án đúng có giải thích: <b>[đáp án-1-giải thích]</b><br/>Đáp án sai: <b>[đáp án]</b> hoặc <b>[đáp án-0]</b></br>Đáp án sai có giải thích: <b>[đáp án-0-giải thích]</b><br/>Nhấn <b>Ctrl + Space</b> để tạo lựa chọn đúng, <b>Shift + Space</b> để tạo lựa chọn sai<br/>VD: I am a man.</br>I {[a][is-0][are-0-giải thích][am-1-giải thích]} a man.<br/></td>';
    html += '                   </tr>';
    html += '               </table>';
    html += '           </div>';
...
function createEventParent(_index) {
    ...
    CKEDITOR.inline('parentTitle' + _index, { language: 'vi'});
...
function resetNumNgay() {
    ...
    $('.addContext').find('.itemNgay').each(function (index) {
        $(this).find('.itemNgay-Title').text('Exercise ' + (index + 1));
    });
}
